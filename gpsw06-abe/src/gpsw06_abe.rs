    ...
    match own_node {
      ...
      AccessNode::Node(thresh, children) => {
        ...
        let children_res: Vec<(u8, G1), consts::U30> = children.iter().enumerate().
          map(|(i, child_ptr)| keygen_node(privkey, pubkey, tree_arr, *child_ptr, &own_poly, F::from((i+1) as u64), rng))
          .flatten()
          .collect();
        return children_res;
      }
  ...
  match own_node {
    ...
    AccessNode::Node(thresh, children) => {
      ...
      let children_result: Vec<(F, Gt), S> = pruned.into_iter()
        .map(|i| (F::from((i) as u64), decrypt_node(tree_arr, children[(i-1) as usize], secret_shares, att_es))) // node indexes start at one, enumerate() starts at zero! 
