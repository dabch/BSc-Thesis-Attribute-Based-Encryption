...
For one, interoperability between the ``light'' pairing library presented here and a potential ``full'' counterpart (e.g. for the \acrshort{kgc}) was not a priority.
...
In \cite{scott_deployment_2020}, the \emph{MIRACL Core} library was evaluated on the same SoC as ours using the same type of curve (256-bit BN curve).
...
This approach requires only a single encryption of \acrshort{abe} for potentially very many messages.
...
However, it was not evaluated for this thesis, because it doesn't represent ``true'' \acrlong{abes}.
