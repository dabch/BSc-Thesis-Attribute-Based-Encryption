...
Their main construction uses \glspl{access-tree} and a small attribute universe, but they also give constructions with a large attribute universe and for \gls{lsss} / \gls{msp} access structures, respectively.
...
The scheme in \cite{lewko_revocation_2008} provides efficient direct revocation. It also relates direct revocation to non-monotone \glspl{access-structure}.
The construction for non-monotone access structures from \cite{lewko_revocation_2008} has keys of size $\mathcal{O}(t)$~\cite{lewko_revocation_2008}, where $t$ is as above.
...
It allows the encryptor to choose the revocation mode~\cite{attrapadung_attribute-based_2009}.
