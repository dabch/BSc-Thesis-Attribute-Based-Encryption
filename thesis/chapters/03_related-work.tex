...
One of major factors for the performance of the implementation of an \acrshort{abes} is the underlying pairing computation.
...
Furthermore, many implementations are not portable due to hand-optimized assembly code or the use of hardware-specific instructions.

One of the first notable implementations was the \emph{Pairing-Based Cryptography Library (PBC)}\footnote{\label{footnotepbc}\url{https://crypto.stanford.edu/pbc/}}~\cite{lynn_implementation_2007}. % TODO verify that this was the first!
...
In \cite{akinyele_self-protecting_2010}, a pairing-based \acrshort{abe} scheme is evaluated on a standard computer and an ARM-based smartphone (iPhone 4). % TODO security level unclear
On the smartphone, only decryption is implemented because encryption is not needed in their scenario.
They conclude that for policies with less than 30 leaves, decryption on a smartphone is feasible (taking between around 2 and 7 seconds, depending on the scheme) \cite{akinyele_self-protecting_2010}.
...
Note that the NEON instruction set is not available on our SoC. % TODO security level 127bit

In \cite{wang_performance_2014}, \glslink{gls-cp-abe}{CP-} and \acrshort{kp-abe} are evaluated for different security levels on an Intel Atom-based smartphone using a Java implementation.
...
% TODO for the last few paragraphs, specify whether only encryption or also keygen/decryption is tested on the microcontrollers!