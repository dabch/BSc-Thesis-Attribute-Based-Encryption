...
Because the collected data is highly sensitive and is transmitted via possibly insecure channels, encryption is vital.
However, simple transport encryption (e.g. by \acrshort{ble} between the sensor and gateway and TLS between the gateway and cloud) is not sufficient because it requires the gateway and the cloud provider to be trusted.
This is not necessarily a reasonable assumption:
