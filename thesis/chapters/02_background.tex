...
This chapter introduces the relevant mathematical background for the \acrshortpl{abes} implemented in this thesis. For further reference, please refer to % TODO
...
\begin{figure}
    ...
    \begin{subfigure}[t]{0.45\textwidth}
        ...
        \begin{tikzpicture}[actor/.style={draw, minimum width=1.5cm}]
            ...
            \node[above, align=center] at (kgc.north) {$\{PK, MK\}$\\$k_A := \text{KeyGen}(PK, MK, S_A)$\\$k_B := \text{KeyGen}(PK, MK, S_B)$};

            \node[below, align=center] at (a.south) {$\{PK, k_{A}\}$\\$\{m, \omega\}$};
            \node[below, align=center] at (b.south) {$\{PK, k_{B}\}$};

            \draw[->] (kgc) edge node [left] {\{$PK$, $k_A$\}} (a);
            \draw[->] (kgc) edge node [right] {\{$PK$, $k_B$\}} (b);

        \end{tikzpicture}
        \caption{If $S_A$ and $S_B$ are allowed access policies for Alice and Bob, respectively, the \acrshort{kgc} creates their private keys and transmits them via a secure channel.}
    \end{subfigure}
    \par\bigskip
    \begin{subfigure}[c]{0.6\textwidth}
        ...
        \begin{tikzpicture}[actor/.style={draw, minimum width=1.5cm}]
            ...
            \node[actor] (kgc) at (0,2.5) {KGC};
        
            \node[above, align=center] at (kgc.north) {$\{PK, MK\}$};

            \node[below, align=center] at (a.south) {$\{PK, k_{A}\}$\\$\{m, \omega\}$\\$c:=\text{Encrypt}(PK, m, \omega)$};
...
In any instance of \acrshort{abe}, there needs to arbiter who decides whether an attribute applies to a certain user or object.
...
\end{figure}
ABE schemes can further be divided into \acrfull{cp-abe} and \acrfull{kp-abe}.
In \acrshort{cp-abe}, so the key is associated with a set of attributes and the ciphertext is encrypted under an access structure.
\acrshort{kp-abe} works the other way around, so the ciphertext is associated with a set of attributes, and the key is associated with an access structure.
...
\subsection{Definition of an \acrshort{abe} Scheme}\label{sec:definition-es}
...
See Figure~\ref{fig:sample-access-tree-shamir} for a sample of an access tree. It is satisfied by any set of attributes that contains two of $A, B$ and either $C$ or $D$.
