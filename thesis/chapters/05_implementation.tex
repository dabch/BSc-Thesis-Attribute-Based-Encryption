...
For the elliptic curve groups (structs \texttt{G1} and \texttt{G1}), additive notation is used and the \texttt{*} operator is conveniently overloaded.
...
Therefore, it was rewritten to introduce a cargo-feature \texttt{std} which controls the inclusion of the standard library and is enabled by default.
If this feature is disabled, the \texttt{core} library and stack-allocated collections of fixed size from the \texttt{heapless}\footnote{\url{https://crates.io/crates/heapless}} crate are used instead.
...
Normal Rust programs use the \texttt{rand} \gls{crate}'s \verb+rand::thread_rng()+ struct to generate random numbers.
This generator relies on the operating system for a random seed and is cryptographically secure~\cite{noauthor_rust_nodate}.

This generator, of course, is unavailable on the SoC.
