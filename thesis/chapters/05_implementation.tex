...
The encryption function therefore just samples a random element from \texttt{Gt}, encrypts it with GPSW and then encrypts the ciphertext using AES with the key derived from the random \texttt{Gt}.

\subsubsection{YCT}
The YCT scheme itself only generates a random curve point; its formal definition relies on a \gls{privkes} to encrypt the data.
...
\subsection{Recursive key generation algorithm}
...
The actual decryption key consists of a set of secret shares, which are created whenever the key generation algorithm hits a leaf node.
...
\subsection{Recursive decryption algorithm}
...
\section{GPSW scheme}
\subsection{Random choice of symmetric key}

\section{YCT scheme}
\subsection{Randomization of \texttt{index()} function}
