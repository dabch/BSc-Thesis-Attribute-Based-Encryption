...
\newglossaryentry{gls-kgc}
{
        ...
        description={trusted central authority that sets up an \acrshort{abes} and generates keys for users of an \acrshort{abes}. See section~\ref{sec:kgc}}
}
...
\newglossaryentry{gls-kp-abe}{
  ...
  description={variant of \gls{abe} where the key is associated with an \gls{access-policy} and the ciphertext is associated with a set of \glspl{attribute}}
}
...
\newglossaryentry{gls-cp-abe}{
  ...
  description={variant of \acrshort{abe} where the ciphertext is associated with an \gls{access-policy} and the key is associated with a set of \glspl{attribute}}
}
...
\newglossaryentry{security-level}{
  ...
  description={
    ...
    to guarantee a security level of $n$\,bits, usually the the size of the field underlying our elliptic curve (i.e. the number of bits needed to store a curve point) needs to be \emph{at least} $2n$, sometimes much larger},
}
...
\newglossaryentry{crate}{
  name={crate},
  description={bundle of Rust code that groups some related functionality together. Can be published to \href{https://crates.io}{\texttt{crates.io}} to share with other developers},
}
...
\newglossaryentry{perfect-binary-tree}{
  name={perfect binary tree},
  description={binary tree in which all inner nodes have exactly two children and all leaves have the same depth}
}

\newglossaryentry{field}{name={field},description={a set together with addition and multiplication operations that behave like they do over the real numbers (formalized by field axioms). Comparable to a \gls{group}, but with two operations instead of one}}

\newglossaryentry{slice}{name={slice},description={Rust data type that represents a variable-length portion of an array. Slices are characterized by a pointer to their beginning and their length}}